...
public class M最短路径_Dijkstra算法 {
    public static void main(String[] args) throws Exception {
        //创建一副加权有向图
        BufferedReader br = new BufferedReader(new InputStreamReader(M最短路径_Dijkstra算法.class.getClassLoader().getResourceAsStream("file_temp/min_route_test.txt")));
...
class DijkstraSP {
    ...
    private void relax(EdgeWeightedDigraph G, int v) {
        for (DirectedEdge edge : G.adj(v)) {
            ...
            //通过松驰技术，判断从起点到顶点w的最短路径是否需要先从顶点s到顶点v，然后在由顶点v到顶点w
            if (distTo(v) + edge.weight() < distTo(w)) {
                distTo[w] = distTo[v] + edge.weight();
                edgeTo[w] = edge;

                //判断pq中是否已经存在顶点w,如果寸在，则更新权重，如果不存在，则直接添加
                if (pq.contains(w)) {
